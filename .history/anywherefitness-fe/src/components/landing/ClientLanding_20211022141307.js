...
const ClientLanding = (props) => {
  ...
  const [clientReservedClasses, setReservedClasses] = useState([]);
  const [allAvailableClasses, set_availableClasses] = useState([]);
  ...
  return (
    <div className="client-landing">
      ...
      {allAvailableClasses.map((each) => {
        return <p>{JSON.stringify(each)}</p>;
      })}
      <p>==============Reserved Classes===============</p>
      {clientReservedClasses.map((each) => {
        return <p>{JSON.stringify(each)}</p>;
      })}
    </div>
  );
